...
def set_placeholder(adj, latent_dim):
    var_placeholders = {
        ...
        'real_distribution': tf.compat.v1.placeholder(dtype=tf.float32, shape=[adj.shape[0], latent_dim],
...
#####################################################################################
def remove_edges_randomly(adj_matrix, remove_ratio):
 
    adj_matrix_coo = adj_matrix.tocoo()
    
    num_nonzeros = adj_matrix_coo.nnz
    
    num_remove = int(num_nonzeros * remove_ratio)
    
    remove_indices = np.random.choice(num_nonzeros, size=num_remove, replace=False)

    mask = np.ones(num_nonzeros, dtype=bool)
    mask[remove_indices] = False
    
    new_data = adj_matrix_coo.data[mask]
    new_row = adj_matrix_coo.row[mask]
    new_col = adj_matrix_coo.col[mask]
    
    modified_adj_matrix = sp.coo_matrix((new_data, (new_row, new_col)), shape=adj_matrix.shape).tocsr()
    
    return modified_adj_matrix
###################################################################

def add_false_edges(adj_matrix, proportion):
   
    rows, cols = adj_matrix.nonzero()
    num_edges = len(rows)
    num_nodes = adj_matrix.shape[0]
    
    num_false_edges = int(num_edges * proportion)
...
def add_edges_to_adj_matrix(adj_matrix, edges):
...
class OptimizerVAE(object):
    def __init__(self, preds, labels, model, num_nodes, pos_weight, norm, d_real, d_fake, learning_rate_1, learning_rate_2):
        ...
        self.optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=learning_rate_1)
        ...
        with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=False):
            ...
            self.discriminator_optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=learning_rate_2,
                                                                 decay=0.9, name='resprop1').minimize(self.dc_loss, var_list=dc_var)
           
            self.generator_optimizer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=learning_rate_2,
                                                             decay=0.9, name='resprop2').minimize(self.generator_loss,
                                                                                              var_list=en_var)
...
def update(model, opt, sess, adj_norm, adj_label, features, placeholders, adj, dropout_rate, latent_dim):
    ...
    #emb_hidden1 = sess.run(model.hidden1, feed_dict=feed_dict)

    emb_hidden2 = sess.run(model.embeddings, feed_dict=feed_dict)

    emb_hidden3 = sess.run(model.embeddings, feed_dict=feed_dict)        ###3##33333
    ...
    return emb_hidden1, emb_hidden2, emb_hidden3, avg_cost
